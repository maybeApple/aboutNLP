...
if ((Test-Path $neo4jBat) -and (Test-Path (Join-Path $javaHome "bin\java.exe"))) {
    ...
    Write-Host "Stopping bundled Neo4j from $neo4jHome"
    & $neo4jBat stop
    exit 0
}
...
try {
    Write-Host "Stopping Neo4j with Docker Compose"
    Invoke-ComposeCommand -Command $composeCommand -Arguments @("stop", "neo4j")
}
finally {
    Pop-Location
}
